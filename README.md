...
Live trading stays **off** unless `.env` has `DRY_RUN=false` **and** `LIVE_TRADING=true` **and** `LIVE_TRADING_CONFIRM=YES_I_ACCEPT_REAL_MONEY_RISK` **and** both API keys. Missing confirm is `LIVE_READY` (full path, `WOULD_SUBMIT_ORDER` only).
