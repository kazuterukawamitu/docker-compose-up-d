...
# Do not chmod from ~ — that is what produces:
#   chmod: start.sh: No such file or directory
# Run this file by path, or bash it after cd into the clone:
...
BOT_BRANCH="cursor/bitbank-btc-jpy-bot-09cf"
...
usage_clone() {
  ...
  echo "  bash \$HOME/docker-compose-up-d/start.sh" >&2
}
...
if [[ -x "$ROOT/.venv/bin/python" ]] && "$ROOT/.venv/bin/python" -c 'import sys; raise SystemExit(0 if sys.version_info >= (3, 12) else 1)'; then
  ...
  if "$PYTHON" -m venv "$ROOT/.venv" && [[ -x "$ROOT/.venv/bin/python" ]]; then
    ...
    echo "隔離したい場合: macOS は brew install python@3.12 / Linux は python3.12-venv を入れてからやり直してください。" >&2
    rm -rf "$ROOT/.venv"
    VENV_PY="$PYTHON"
  fi
fi
